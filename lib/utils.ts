...
export function deriveDropdownValues(data: SafeForDropdown[] | any[], t: any, useTranslation: boolean) {
  const emptyDropdownValues = [{ label: t("other"), value: t("other") }];
  if (!data) return emptyDropdownValues;
  let nomalizedData = data.map((item) => {
    if (typeof item === 'number') {
      return item.toFixed(0);
    }
    return useTranslation ? t(item) : item;
  });
